...
export const fetchPlayersOverTime = createServerFn()
  ...
  .handler(async ({ data: { typeCode } }) => {
    ...
    const factionData = await db
      ...
      .where("ranking_snapshot_event.player_id", "in", [
        ...new Set(playerData.map((r) => r.player_id!)),
      ])
      .select([
    ...
    const grouped = new Map<
      string,
      { date: string; players: typeof playerData }
    >();
...
export const fetchPlayerPaintingWins = createServerFn()
  ...
  .handler(async ({ data: { playerId } }) => {
    ...
    const totalsByCat = new Map<number, number>(
      totals.map((t) => [t.categoryId, t.totalWinners]),
    );
    return wins.map((w) => ({
      ...w,
...
export const fetchTourney = createServerFn()
  ...
  .handler(async ({ data: { id } }) => {
    const [players, tourney, paintingCategoryRows] = await Promise.all([
      ...
      db
        ...
        .leftJoin(
          "image",
          "image.key" as any,
          "painting_winner.image_key" as any,
        )
    ...
    const paintingCategories = (paintingCategoryRows as any[]).reduce(
      (acc: any[], row: any) => {
...
export const fetchTeamsOverTime = createServerFn()
  ...
  .handler(async ({ data: { typeCode } }) => {
    const rows = await db
      .selectFrom("team_ranking_snapshot")
      .innerJoin(
        "team_ranking_snapshot_batch",
        "team_ranking_snapshot.batch_id",
        "team_ranking_snapshot_batch.id",
      )
      .innerJoin("team", "team_ranking_snapshot.team_id", "team.id")
...
export const fetchRegionsOverTime = createServerFn().handler(async () => {
  ...
  const groupedByDate = rows.reduce(
    (acc, row) => {
      ...
      if (!acc[dateKey])
        acc[dateKey] = { dateKey, batchId: row.batch_id, regions: [] };
      if (row.batch_id >= acc[dateKey].batchId) {
        acc[dateKey] = {
          dateKey,
          batchId: row.batch_id,
          regions:
            row.batch_id > acc[dateKey].batchId ? [] : acc[dateKey].regions,
        };
      }
...
export const fetchAllPainting = createServerFn().handler(async () => {
  const rows = (await db
    ...
    .select([
      ...
      "painting_winner.description" as any,
      "painting_winner.position",
      sql<number>`count(*) over (partition by ${sql.ref("painting_winner.category_id")})::int`.as(
        "totalWinners",
      ),
    ])
    .orderBy("tourney.date", "desc")
