...
export function PaintingLightbox({
  ...
  return (
    <Modal
      ...
      size="xl"
      centered
      title={winner ? `${winner.categoryName} — ${positionLabel(winner.position, winner.totalWinners)}` : ''}
    >
      {winner && (
        <Box>
          {winner.imageKey && (
            <Image
              ...
              radius="sm" fit="contain" mah={500} mb="md"
            />
          )}
          {linkPlayerName && winner.playerId != null ? (
            <Link to="/player/$id" params={{ id: winner.playerId }} search={{ tab: 'painting', typeCode: undefined, painting: undefined }} fw={600}>
              {winner.playerName}
            </Link>
          ) : (
            <Text fw={600}>{winner.playerName}</Text>
          )}
          {winner.model && <Text size="sm" c="dimmed">{winner.model}</Text>}
          {winner.description && <Text size="sm" mt="xs">{winner.description}</Text>}
        </Box>
      )}
    </Modal>
  )
