...
describe('playerShortName', () => {
  ...
  it('returns first word for multi-word names when short_name is null', () => {
    expect(playerShortName({ name: 'Jonathan Smith', short_name: null })).toBe('Jonathan')
  })
  ...
  it('truncates to 8 chars for single long names when short_name is null', () => {
    expect(playerShortName({ name: 'Maximilian', short_name: null })).toBe('Maximili')
  })
})
